...
if(isset($_GET['cno']))
{ $cno=$_GET['cno']; $query = "SELECT * FROM ads WHERE status = 'approved' and cate=".$cno; }
else{ $query = "SELECT * FROM ads WHERE status = 'approved'"; }
	
		if(isset($_POST['srh']))
		{ 
			$sr=$_POST['srh'];
			$aq=mysqli_query($conn,"SELECT * FROM `tags` where `tags`='$sr'");
			//$count=mysqli_num_rows($aq);echo $count;
			while($re=mysqli_fetch_array($aq))
			{
				$id=$re['adid'];
				$query = "SELECT * FROM ads WHERE status = 'approved' and id=".$id;
			}
		}
...
<marquee direction="left" style="color:blue; font-weight:bold; font-size:2em;">Community Connect - Local Social Network - Post Your Ads Now ...!</marquee>
 <button type="button" class="btn btn-primary" data-bs-toggle="collapse" data-bs-target="#demo">Show Ad Categories</button>
  <div id="demo" class="collapse">
  <div class="row">
  <?php
  $qry=mysqli_query($conn,"SELECT * from category") or die(mysqli_error());
  while($rs=mysqli_fetch_array($qry))
  {
  ?>
    <div class="col-sm-2">
	<a href="index.php?cno=<?php echo $rs['sno']; ?>" class="btn btn-warning"><?php echo $rs['cname']; ?></a>
	</div>
	<?php
     }
    ?>
    ...
    <main>
		...
		<div class="container">
		<h5 style="padding:10px;"><u>Premium Ads</u></h5>
        <?php 
		...
		if (mysqli_num_rows($result) > 0): ?>
            <div class="row">
                <?php while ($ad = mysqli_fetch_assoc($result)): ?>
					...
					<img src="adimages/<?php echo $ad['img']; ?>" alt="img" height="120" style="width:99%; padding:1px;" /></a>
					<hr>
